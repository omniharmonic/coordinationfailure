    ...
    async (args) => {
      ...
      const help: Record<string, string> = {
        ...
        ai_dilemma: [
          ...
          '  + 10 per honored agreement, -20 per violated agreement',
          '',
          'GOVERNMENTS:',
        ...
        if (gameType === 'ai_dilemma') {
          ...
          for (const role of rolesToFill) {
            ...
            const prompt = [
              ...
              isCompany ? '+ 10 per honored agreement, -20 per violated' : '+ 15 per honored agreement',
              '',
              'You are RACING to be first to AGI — but if anyone arrives with alignment < 60, everyone gets NEGATIVE scores.',
              '',
              isCompany
                ? `STRATEGY: You want to WIN, not just survive. Race for AGI but keep alignment above 60. Start safety at 0.3-0.4 for speed. At capability 80+, raise to 0.6-0.8 or alignment collapses. DM your government for subsidies. Propose safety pacts (each honored = +10 to your score). Create GROUP CHATS for coalitions. Watch rivals — if someone is racing recklessly, lobby their government or call them out. You need to be FIRST.`
                : `STRATEGY: Your score depends on your domestic companies' alignment. Fund the ones investing in safety. Regulate to prevent reckless racing but don't cripple growth. Spy on ${foreignGov} to monitor foreign progress. Honored agreements = +15 points each. Create GROUP CHATS with your domestic companies. Consider a cross-border group with ${foreignGov} for international frameworks. Your companies need to WIN the race — help them do it safely.`,
            ].filter(Boolean).join('\n');
